...
export default function ChatReviewItem(props: ReviewItemProps) {
  ...
  return (
    <li className="mb-7">
      <div className="ml-2.5 flex gap-0.5 items-center w-fit">
        <Star
          setStar={() => {
          }}
          star={props.rate}
      ...
      <div className="ml-2.5 w-fit" css={css`font-size: 0.6rem;`}>
        ...
        {style.reviewLike.buttonType !== 'NONE' ?
          <button onClick={(evt) => {
            ...
            if (userID === props.reviewerID) {
              console.log('자신의 리뷰에는 도움돼요 버튼을 누를 수 없어요!');
              return;
            }
            console.log('도움돼요!');
          }}
            ...
            {style.reviewLike.buttonType === 'THUMB_UP_WITH_TEXT' ?
              <div className="ml-1">도움돼요!</div> : null
            }
