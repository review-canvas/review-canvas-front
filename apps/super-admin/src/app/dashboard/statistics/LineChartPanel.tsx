...
function init_score_chart( criteria : string )  {
    ...
    const chartStatus = Chart.getChart('line_chart');
    if (chartStatus !== undefined)
        chartStatus.destroy();
    const canvasElement = document.getElementById('line_chart') as HTMLCanvasElement;
    // @ts-ignore
    ...
    return (
        <div style={{ width: '500px', backgroundColor: 'white', borderRadius: '10px', padding: '20px', margin: '30px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
            ...
            <div style={{display: 'flex', justifyContent: 'center'}}>
                <canvas id="line_chart" style={{border: '1px solid #CCCCCC'}}></canvas>
            </div>
        </div>
    );
}
